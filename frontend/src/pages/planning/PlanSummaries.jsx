    ...
    <div className="space-y-4">
      <Section title="Plan Summary">
        <table className="w-full">
          <thead><tr>
            <th className="th">Product</th><th className="th">Plan ID</th><th className="th text-right">Target Qty</th>
            <th className="th text-right">Executed Qty</th><th className="th text-right">Remaining Qty</th>
            <th className="th text-right">Batches to Run</th><th className="th">Status</th>
          </tr></thead>
          <tbody><tr>
            ...
            <td className="td num font-semibold">{fmtQty(p.remaining_qty)} {p.uom}</td>
            <td className="td num">{p.remaining_batches}</td>
      ...
      <Section title="Material Summary"
        note={`Requirement for the remaining quantity${applyScrap ? ', incl. scrap allowance' : ', without scrap allowance'} · ${shortCount ? `${shortCount} material(s) short` : 'all materials available'}`}>
        <table className="w-full">
          <thead><tr>
