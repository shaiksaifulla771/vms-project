 ...
 `);
  ...
  const handleExportData = () => {
    const dataToExport = filteredMaterials.map(m => ({
      "Material Name": m.name, "Material Code": m.code, "UOM": m.unit, "Category": m.type, "Sub Category": m.subcategory || '', "Status": m.status || 'Active', "Description": m.description || ''
    }));
    ...
    <div className="space-y-3 w-full">
          ...
          <div className="flex items-center space-x-1.5 w-full md:w-auto shrink-0 justify-end">
            ...
            <div className="relative z-[100]">
              ...
              >
                ...
                <span className={`text-[9px] transition-transform duration-200 ${showFunctionList ? 'rotate-180' : ''}`}>▼</span>
              </Button>

              {showFunctionList && (
                <>
                  <div className="fixed inset-0 z-[90] cursor-default" onClick={() => setShowFunctionList(false)} />
                  <div className="absolute right-0 top-full mt-1.5 w-56 bg-white border border-slate-200/90 rounded-lg shadow-2xl z-[100] py-1.5 text-left ring-1 ring-black/5 animate-in fade-in slide-in-from-top-2 duration-150">
                    ...
                    <button
                      onClick={() => { setShowFunctionList(false); handleOpenAddModal(); }}
                    ...
                    >
                      ...
                      <div className="flex flex-col text-left">
                        ...
                        <span className="text-[10px] text-slate-400 font-normal mt-0.5">Add single new material record</span>
                      </div>
                    </button>

                    <button
                      onClick={() => { setShowFunctionList(false); setImportSummary(null); setIsAutoEntry(true); setIsImportModalOpen(true); }}
                    ...
                    <button
                      onClick={() => { setShowFunctionList(false); handleExportData(); }}
                ...
                <div className="space-y-2 p-3 border border-amber-200 rounded-md bg-amber-50/50">
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-1.5">
                    {editableAcceptedItems.map((item, idx) => {
                      ...
                      return (
                        <div key={idx} className={`flex items-center justify-between px-3 py-2 rounded border text-xs font-semibold transition-all ${isConfirmed ? 'bg-emerald-50 border-emerald-200 text-emerald-800' :
                          ...
                          <div className="flex items-center space-x-1.5 ml-3 flex-shrink-0">
                            {(isConfirmed || isSkipped) ? (
                              <>
                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isConfirmed ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
                                  }`}>
                  ...
                  <div className="space-y-2.5 p-3 border border-blue-200 rounded-lg bg-gradient-to-b from-blue-50/60 to-white">
                    ...
                    {totalNeedingAction > 0 && (
                      <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${allResolved ? 'bg-emerald-500' : 'bg-blue-500'
                            }`}
                        ...
                        <>
                            ...
                            <div className="flex items-center gap-1 flex-wrap">
                              {tabs.map(tab => (
                                <button
                                  ...
                                  className={`text-[10px] font-bold px-2.5 py-1 rounded border transition-all ${bulkUpdateTab === tab.key ? tab.active + ' ' + tab.color : 'bg-white text-slate-400 border-slate-200 hover:border-slate-300'
                                    }`}
                                >
                                  {tab.label}
                                </button>
                          ...
                          <div className="max-h-[58vh] overflow-y-auto space-y-2 pr-0.5">
                            ...
                            {filteredItems.map(({ item, idx }) => {
                              ...
                              return (
                                <div key={idx} className={`rounded-lg border text-xs transition-all ${item.isExistingMatch
                                  ...
                                  <div className="flex items-center justify-between px-3 py-2">
                                    <div className="flex items-center gap-2">
                                      <span className={`font-bold ${isSkipped ? 'line-through text-slate-400' : 'text-slate-800'
                                        }`}>{item.name}</span>
            ...
            <div className="grid grid-cols-2 gap-3.5">
              ...
              <div className="flex flex-col space-y-1.5">
                ...
                <div className="h-7 flex items-center">
                  <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider border ${viewingMaterial.status === 'Active'
      ...
      <div className="fixed top-4 right-4 z-[9999] space-y-2 pointer-events-none">
        {toasts.map(t => (
          <div
            key={t.id}
            className={`pointer-events-auto flex items-center justify-between space-x-2 px-3.5 py-2.5 rounded-lg shadow-xl border text-xs font-extrabold transition-all duration-300 transform translate-y-0 opacity-100 ${t.type === 'success'
