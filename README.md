...
0. **Master data** - Materials (code M1001.., category / sub-category, description, status), Vendors (V1001..,
   ...
   **Option lists (v5):** each category belongs to a classification, so the Material form and bulk template offer
   only that classification's categories, then that category's sub-categories; every UOM field is a dropdown from
   Settings > UOMs (case-insensitive, checked in the database). **Products** lists finished / semi-finished goods with
   active BOM, cost per unit, stock and open plans. Company set-up lives in **Settings**: Locations & WH, Categories, UOMs.
