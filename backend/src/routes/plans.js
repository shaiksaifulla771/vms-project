...
async function buildPlanView(db, planId) {
  ...
  const remaining = Number(plan.remaining_qty);
  ...
  return {
    ...
    planSummary: {
      ...
      remaining_batches: active ? planning.requiredBatches(remaining, bom.expected_output_qty) : 0,
      status: plan.status,
      uom: plan.output_uom,
    },
    batchSummary,
...
router.post('/simulate', h(async (req, res) => {
  ...
  const demand = v.num(b.demand_qty, 'Required quantity', { required: true, gt: 0 });
  const db = getPool();
  ...
  const batches = planning.requiredBatches(demand, bom.expected_output_qty);
  res.json({
    bom,
    apply_scrap_allowance: applyScrap,
    planSummary: {
      product: `${bom.product_code} - ${bom.product_name}`, plan_no: null, target_qty: demand,
      executed_qty: 0, remaining_qty: demand, remaining_batches: batches, status: 'SIMULATION', uom: bom.output_uom,
    },
...
router.post('/', h(async (req, res) => {
  ...
  const id = await withTransaction(async (c) => {
    ...
    const bom = (await c.query('select warehouse_id from public.boms where id = $1', [bomId])).rows[0];
    ...
    const r = await c.query(`insert into public.plans(product_id, location_id, warehouse_id, bom_id, target_qty,
                               apply_scrap_allowance, required_date, notes, created_by)
                             values ($1,$2,$3,$4,$5,$6,$7,$8,$9) returning id`,
    [productId, locationId, warehouseId, bomId, target, applyScrap,
      v.date(b.required_date, 'Required date'), v.str(b.notes, 'Notes', { max: 2000 }), req.user.id]);
    await c.query(`insert into public.plan_events(plan_id, event, new_value, user_id) values ($1,'CREATED',$2,$3)`,
      [r.rows[0].id, JSON.stringify({ target_qty: target }), req.user.id]);
    return r.rows[0].id;
  });
